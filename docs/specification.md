...
iteration is a **sprint**: pick a focus area from the vision (or from the
human feedback waiting in the inbox, §8.9), write a spec, break it into
tickets, implement and review each ticket, UX-test the
...
docs/
  ...
  feedback/                # optional; scaffolded by init (§8.9)
    README.md              # what the two folders are for
    inbox/                 # human drops feedback here, one .md per item
    handled/               # the loop archives triaged items here (NN-<name>.md)
...
- Sprint folders: `NN-slug` — two-digit zero-padded number + kebab-case slug,
  ...
  `sprint_focus.md` — the folder name is the focus; a **feedback sprint**
  (§8.9) is exactly `NN-feedback` and its `sprint_focus.md` carries the
  feedback stamp. `cleanup` and `feedback` are therefore reserved slugs for
  normal sprints (§6.1).
...
- `docs/feedback/README.md`, `docs/feedback/inbox/.gitkeep`, and
  `docs/feedback/handled/.gitkeep` (§8.9) — the folders are optional, so
  they are scaffolded but never required by validation
...
A newly *created* sprint is a **feedback sprint** (§8.9) whenever
`docs/feedback/inbox/` holds at least one item — a non-empty `*.md` file
directly in the folder. The inbox is read at the sprint boundary (a fresh
read, after any merge), so feedback dropped mid-sprint waits for the next
sprint rather than shifting the running one's scope.

Otherwise, two independent triggers make a newly created sprint a **cleanup
sprint** (§8.8):

- **Flag**: `--cleanup` forces the first sprint created during the run. If
  the run creates no cleanup sprint — it only resumed and finished an
  in-progress sprint, or feedback outranked the flag — the orchestrator
...
**Feedback outranks both cleanup triggers** — it is the freshest human
input, and the human can always empty the inbox — and it does **not** consume
a pending `--cleanup`, which still applies to the next sprint the run
creates (a run that creates none says so and the flag is simply lost). The
cadence stays purely number-derived: a feedback sprint at `NN` does not
shift the schedule, which also means the cleanup that `NN` would have been
...
**Feedback sprints** (§8.9): a latest sprint whose folder matches
`^\d{2}-feedback$` replaces only the `sprint_focus.md` row; from `spec.md`
on it derives exactly like a normal sprint:

| Observed state | Next phase |
|---|---|
| No `sprint_focus.md`, its first line is not a valid feedback stamp, or the stamp says `triaged=no` | step F2 — a failed F2 postcondition or a crash mid-step (retry policy §6.3), never a derivation crash |
| `triaged=yes`, `actionable=none` | the shared tail with zero tickets — the UX rows, then report and vision status |
| `triaged=yes`, `actionable=yes` | steps 3 → 4 → the shared tail, unchanged |

`feedback` is a **reserved slug** on the same terms as `cleanup`. The agent
writes `triaged=no` and the orchestrator flips it to `yes` once the items
are archived (§8.9) — the same agent-writes-no idiom as the UX ticketized
stamp — so a focus file the postcondition rejected can never be mistaken for
a finished triage. An `actionable=none` sprint never writes a `spec.md` or a
ticket: like an all-`none` cleanup sprint it lands straight on the UX rows,
so the loop never manufactures work to fill a sprint the triage found
nothing for.
...
- **Feedback focus**: a feedback sprint's `sprint_focus.md` starts with
  `_Feedback: triaged=<no|yes>, actionable=<yes|none>,
  vision-change=<proposed|no>_`; the orchestrator flips `triaged` after
  archiving, as it flips the UX findings stamp.
...
- The **orchestrator** commits loop artifacts deterministically after each
  step: `chore(loop): sprint NN focus` (`chore(loop): sprint NN feedback
  focus` on feedback sprints — the commit carries the archive move too,
  §8.9), `chore(loop): sprint NN spec`,
...
| F2 feedback triage | `claude-fable-5` | extra | 50 |
...
step 6 and is **not** validated here, and neither are the feedback folders
(§8.9) — they are optional, and a missing inbox simply reads as empty.
...
### 8.9 Feedback sprints — step F2

A third sprint type: instead of advancing the vision, a **feedback sprint**
handles what a human actually said. Two folders sit next to `docs/vision.md`
— `docs/feedback/inbox/` and `docs/feedback/handled/` — and while the inbox
holds an item, the next sprint the loop creates is a feedback sprint
(`NN-feedback`). Step **F2** replaces step 2; everything from step 3 on is
the existing machinery, unchanged. Why: the loop's only human inputs were
the north-star `vision.md` and unblocking a ticket, so ordinary feedback had
nowhere to go that the loop reads — and reusing step 2's prompt would
re-derive the answer from the very vision the feedback may be disputing.
Triggering and precedence live in §5; on-disk representation and derivation
in §6.1.

#### The folders

- An **inbox item** is a regular `*.md` file **directly** in
  `docs/feedback/inbox/` with something other than whitespace in it.
  Dotfiles, other extensions, subdirectories, symlinks (the archive would
  hold a link instead of the human's words), and names containing a newline
  (no `### <filename>` heading could name them) are all ignored by the
  trigger and by the archive — so a `.gitkeep` costs nothing and a stray
  `touch` cannot start a sprint.
- Item order is filename order, and the filename is the item's handle
  everywhere: prompt, `sprint_focus.md`, archive name.
- Both folders are optional (§8.1) and tracked in git like the rest of
  `docs/`. `init` scaffolds them (§5).

#### F2 — Triage feedback (agent; replaces step 2)

**Purpose**: decide what sprint `NN` should do about the feedback, and what
the feedback means for the vision.

- **Inputs**: every inbox item — path **and** full text, inlined in the
  prompt by the orchestrator — plus `docs/vision.md` (read-only),
  `.working/vision_status.md`, `docs/CONTEXT.md`, and free exploration of
  the codebase.
- **Exactly one disposition per item**:
  1. **`in-vision`** — the vision already covers this; the gap is execution
     (not built, built badly, buggy, confusing). This is the material the
     sprint focus is made of.
  2. **`vision-change`** — the vision does not cover this, or the feedback
     contradicts it. **`docs/vision.md` is off-limits to the loop** (§8.8):
     the item becomes a written proposal for the human, and only the part
     the *current* vision already supports may become work.
  3. **`declined`** — out of scope, contradicts a stated non-goal, already
     done and verified, or not worth acting on; the reason is written down.
- **Output**: `sprint_focus.md` in the sprint folder — the same artifact a
  normal sprint's step 2 writes, so steps 3–7 need no knowledge of feedback
  sprints. Its **first line** is the machine-readable feedback stamp:

  ```
  _Feedback: triaged=no, actionable=yes, vision-change=proposed_
  ```

  Exact format: the three keys in that order. `triaged` is
  orchestrator-owned — the agent always writes `no` and the orchestrator
  flips it to `yes` after archiving (below), exactly as it flips the UX
  findings stamp. `actionable=none` — no work for this sprint — is a valid,
  successful outcome (derivation then skips steps 3 and 4, §6.1), correct
  only when no item is `in-vision`. The body carries a `## Feedback` section
  with one `### <filename>` subsection per item (disposition, reasoning,
  what it implies, and whether it is taken into this sprint or deferred), a
  `## Vision proposals` section when any item is a `vision-change`, and the
  usual `## What` / `## Why` focus — conservative and small, exactly as step
  2's is. `## What` may contain only work the current vision already
  supports: the sprint that proposes a vision change must not also build it,
  and step 3's prompt is told the proposals are out of scope.
- **Postcondition**: `sprint_focus.md` exists and is non-empty; its first
  line parses as the stamp with `triaged=no`; **every inbox item has its own
  `### <filename>` section carrying exactly one valid
  `- Disposition:` line** (exact heading match — a substring test would let
  `cli.md` be "mentioned" by a block about `slow-cli.md`); the stamp agrees
  with those dispositions (`vision-change=proposed` iff an item is
  `vision-change`; `actionable=yes` iff an item is `in-vision`); and
  `docs/feedback/` **and `docs/vision.md` are untouched** — the triage reads
  them, and an agent that could add an inbox item could hand itself the next
  sprint, while one that could edit the vision would end the feature's whole
  promise. No item may be silently dropped, since it is about to be
  archived.
  A focus the postcondition rejects stays on disk for the human to read, but
  the orchestrator strips any `triaged=yes` off it first: the flip is its
  word that F2 finished, and a rerun must re-triage rather than walk past a
  failed step.
- **Empty inbox at triage time**: if the items are gone by the time F2 runs
  (the human withdrew them after the boundary; a crash left the folder
  behind), the orchestrator writes the focus itself — stamped
  `triaged=yes, actionable=none` and saying so — and spawns no agent.
  Deterministic where possible (§2).

#### Archiving (orchestrator-owned)

Once F2's postcondition passes, the **orchestrator** — never the agent — in
this order: flips the stamp to `triaged=yes`; moves each item from
`docs/feedback/inbox/<name>.md` to `docs/feedback/handled/NN-<name>.md`,
creating `handled/` if needed and never overwriting a name already there
(`NN-<name>-2.md`, …, because the archive holds human words); and commits
both as `chore(loop): sprint NN feedback focus`. The `NN-` prefix records
which sprint handled the item.

The flipped stamp is what derivation reads, and it is written **before** the
move, so every crash window costs at most repeated work and never a lost or
half-recorded triage:

| Dies… | On rerun |
|---|---|
| before the flip | the stamp still says `triaged=no` → F2 runs again on the untouched inbox |
| between flip and move | derivation moves on; the leftover items are triaged — and possibly implemented — a second time by the next feedback sprint |
| between move and commit | the sprint continues; the move is in the working tree but uncommitted — no later step's pathspec covers `docs/feedback/`, so `git status` is what shows it |

The commit skips `docs/feedback/` when a project gitignores it: the archive
on disk is what the loop depends on, and `git add` on an ignored path is a
hard error.

Items are archived at **triage** time, not at sprint completion: the sprint
folder, the report and the archive already record what happened to each
item, and a blocked sprint must not re-trigger itself on its own feedback.
An in-vision item the focus could not fit into this sprint is therefore
archived too — which is why the triage must mark it deferred, and why step 7
carries deferrals into "Known gaps".

#### Unchanged machinery, small deltas

- **Steps 3, 4, 5.x, 5.5**: unchanged. Feedback work is feature work — the
  implement agent commits `feat(sNN/TTT)` as usual.
- **Report (§8.6)**: unchanged postcondition. The orchestrator parses the
  dispositions and puts them in the prompt's sprint-type section, so the
  agent is told what was decided rather than asked to find it, and is told
  to render a proposed vision change as prominently as a blocked ticket —
  only a human can accept it. The same summary goes to the console when F2
  finishes.
- **Vision status (§8.7)**: runs unchanged; its prompt gains two rules —
  a proposed vision change belongs under "Blocked on human" and is carried
  there **every sprint**, feedback sprint or not, until `docs/vision.md`
  reflects it or the human drops it (the loop's only way to keep asking);
  deferred feedback items belong under "Known gaps".
- **Outer loop, stop conditions, exit codes, branches (§5, §6.4, §9)**:
  unchanged. A feedback sprint runs on `sprint/NN` and merges like any
  other.
...
- Feedback that interrupts a running sprint (boundary-only, by design)
- The loop editing `docs/vision.md` — feedback yields proposals, never edits
- Structured feedback items (severity/author/priority fields) or an
  item→ticket traceability index beyond the dispositions in
  `sprint_focus.md`; re-opening a handled item is a human moving the file
  back into `inbox/`
